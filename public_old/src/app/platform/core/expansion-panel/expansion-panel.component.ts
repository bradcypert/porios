...
export class TdExpansionPanelComponent {
  ...
  /**
   * icon?: string
   * Sets icon of [TdExpansionPanelComponent] header.
   */
  @Input() icon: string;

  /**
   * iconCallback?: EventEmitter
   * Sets the callback for the icon click event on the [TdExpansionPanelComponent] header.
   */
  @Output() iconCallback: EventEmitter<any> = new EventEmitter();
  ...
  private _iconClickEvent(event: MouseEvent): void {
    event.stopImmediatePropagation();
    event.preventDefault();
    
    this.iconCallback.emit(true);
  }
}
